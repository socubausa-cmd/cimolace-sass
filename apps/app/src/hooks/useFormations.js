 ...
 * Les colonnes supplémentaires (cycle, duration_weeks, image_url, meta) ont été
 * ajoutées via migration 20260528170001.
 */
...
const normalizeFormationForUI = (row) => {
  ...
  const standalonePriceRaw =
    meta.standalone_price ?? meta?.access?.standalone_price ?? row.price_cents ?? null;
  ...
  return {
    ...
    // price_cents stored as raw amount (no ÷100 — XAF has no subdivisions in this system)
...
const pickDbPayload = (formation) => {
  ...
  return {
    ...
    // Store raw amount as price_cents (XAF stored 1:1, no centimes conversion)
    price_cents: normalizedPrice != null ? Number(normalizedPrice) : null,
...
const COURSES_SELECT =
  'id, title, description, status, cycle, duration_weeks, price_cents, image_url, meta, category, created_at, updated_at';
...
export const useFormations = () => {
  ...
  const refresh = useCallback(async (options = {}) => {
    ...
    for (let i = 0; i < attempts.length; i += 1) {
      ...
      try {
        result = await withTimeout(runQuery(), timeoutMs, 'courses_load_timeout');
        lastThrown = null;
        break;
      } catch (e) {
        const err = e instanceof Error ? e : new Error(String(e));
        lastThrown = err;
        if (String(err?.message || err) !== 'courses_load_timeout') break;
      }
    }

    if (!result) {
      const err = lastThrown instanceof Error ? lastThrown : new Error(String(lastThrown || 'courses_load_timeout'));
    ...
    try {
      window.localStorage.setItem('courses_admin_cache_v1', JSON.stringify(normalized));
    } catch {
      // ignore
    }
    setLoading(false);
  ...
  useEffect(() => {
    let hadCache = false;
    try {
      const raw = window.localStorage.getItem('courses_admin_cache_v1');
      if (raw) {
