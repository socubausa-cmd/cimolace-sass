...
// Grille « atouts » (d'après features-8, 21st.dev) — JSX, charte PRORASCIENCE (or/sombre).
// Cartes = panneaux du thème, SVG décoratifs recolorés en or, avatars = photos du temple.
const TEMPLE = (n) => `/ngowazulu/temple-${String(n).padStart(2, '0')}.jpg`;

const Card = ({ className = '', children }) => (
  <div className={`rounded-2xl border ${className}`} style={{ background: 'var(--panel)', borderColor: 'var(--border)' }}>
    {children}
  </div>
);
...
export function FeaturesAtouts({ eyebrow = "Pourquoi l'ISNA", title = 'Ce qui nous distingue.' }) {
  return (
    <section className="relative py-20 md:py-28">
      <div className="mx-auto max-w-5xl px-6">
        <div className="mb-12 text-center">
          <p className="text-[12px] font-semibold uppercase tracking-[0.4em]" style={{ color: 'var(--gold)' }}>{eyebrow}</p>
          <h2 className="mq-display mt-4 text-4xl font-semibold sm:text-5xl" style={fg}>{title}</h2>
        </div>
        ...
        <div className="relative z-10 grid grid-cols-6 gap-3">
          ...
          <Card className="col-span-full flex overflow-hidden lg:col-span-2">
            <CardContent className="relative m-auto size-fit pt-6">
              <div className="relative flex h-24 w-56 items-center">
                <svg className="absolute inset-0 size-full text-[#39332a]" viewBox="0 0 254 104" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <span className="mx-auto block w-fit text-5xl font-semibold" style={{ color: 'var(--gold)' }}>2500+</span>
              </div>
              <h2 className="mt-6 text-center text-3xl font-semibold" style={fg}>Initiés formés</h2>
            </CardContent>
          </Card>
          ...
          <Card className="col-span-full overflow-hidden sm:col-span-3 lg:col-span-2">
            <CardContent className="pt-6">
              ...
              <div className="relative z-10 mt-14 space-y-2 text-center">
                <h2 className="text-lg font-medium" style={fg}>Progression certifiée</h2>
                <p style={muted}>Une montée en compétence traçable, validée par étapes.</p>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="grid pt-6 sm:grid-cols-2">
              <div className="relative z-10 flex flex-col justify-between space-y-12 lg:space-y-6">
                <div className="relative flex aspect-square size-12 rounded-full border border-white/10 before:absolute before:-inset-2 before:rounded-full before:border before:border-white/5">
                  <Shield className="m-auto size-5" strokeWidth={1} style={{ color: 'var(--gold)' }} />
                </div>
                <div className="space-y-2">
                  <h2 className="text-lg font-medium" style={fg}>LIRI · immersion à distance</h2>
                  <p style={muted}>Cours en direct, présence réelle, suivi contextuel — comme en présentiel.</p>
                </div>
              </div>
              <div className="relative -mb-6 -mr-6 mt-6 h-fit rounded-tl-2xl border-l border-t p-6 py-6 sm:ml-6" style={{ borderColor: 'var(--border)' }}>
          ...
          {/* 5 — communauté encadrée */}
          <Card className="col-span-full overflow-hidden lg:col-span-3">
            <CardContent className="grid h-full pt-6 sm:grid-cols-2">
              <div className="relative z-10 flex flex-col justify-between space-y-12 lg:space-y-6">
                <div className="relative flex aspect-square size-12 rounded-full border border-white/10 before:absolute before:-inset-2 before:rounded-full before:border before:border-white/5">
                  <Users className="m-auto size-6" strokeWidth={1} style={{ color: 'var(--gold)' }} />
                </div>
                <div className="space-y-2">
                  <h2 className="text-lg font-medium" style={fg}>Communauté encadrée</h2>
                  <p style={muted}>Un cadre protégé, une discipline collective, une responsabilité mutuelle.</p>
                </div>
              </div>
              <div className="relative mt-6 before:absolute before:inset-0 before:mx-auto before:w-px before:bg-[var(--border)] sm:-my-6 sm:-mr-6">
                <div className="relative flex h-full flex-col justify-center space-y-6 py-6">
                  {[
                    { n: 'Aïcha M.', src: TEMPLE(6), right: true },
                    { n: 'Karim D.', src: TEMPLE(4), right: false },
                    { n: 'Joseph N.', src: TEMPLE(9), right: true },
                  ].map((p) => (
                    p.right ? (
                      <div key={p.n} className="relative flex w-[calc(50%+0.875rem)] items-center justify-end gap-2">
                        <span className="block h-fit rounded border px-2 py-1 text-xs" style={{ borderColor: 'var(--border)', background: 'var(--bg)', color: 'var(--muted)' }}>{p.n}</span>
                        <div className="size-7 ring-4 ring-[#16120c]">
                          <img className="size-full rounded-full object-cover grayscale" src={p.src} alt="" />
                        </div>
                      </div>
                    ) : (
                      <div key={p.n} className="relative ml-[calc(50%-1rem)] flex items-center gap-2">
                        <div className="size-8 ring-4 ring-[#16120c]">
                          <img className="size-full rounded-full object-cover grayscale" src={p.src} alt="" />
                        </div>
                        <span className="block h-fit rounded border px-2 py-1 text-xs" style={{ borderColor: 'var(--border)', background: 'var(--bg)', color: 'var(--muted)' }}>{p.n}</span>
                      </div>
