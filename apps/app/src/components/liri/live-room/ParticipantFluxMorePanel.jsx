      ...
      <div className="grid grid-cols-3 gap-2 max-h-[38vh] overflow-y-auto pr-1">
        {filtered.map((p) => (
          <div key={p.id} className="space-y-1">
            ...
            />
            <button type="button" onClick={() => { onPromote?.(p.id); onClose?.(); }} className="h-8 w-full rounded-lg bg-white/[0.04] border border-white/10 hover:border-[color-mix(in_srgb,var(--school-accent)_35%,transparent)] hover:text-[var(--school-accent)] text-[11px] text-gray-300">Monter</button>
          </div>
        ))}
      </div>
    </div>
