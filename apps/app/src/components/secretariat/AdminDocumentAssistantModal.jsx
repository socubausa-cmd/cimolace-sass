...
export default function AdminDocumentAssistantModal({ open, onClose, onApply }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div
        className="relative w-full max-w-lg rounded-2xl border border-[var(--lt-border)] bg-[var(--lt-card-bg)] shadow-2xl"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="flex items-center justify-between border-b border-[var(--lt-border)] px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <h2 id="admin-assistant-title" className="text-sm font-semibold text-[var(--lt-text)]">
          ...
          <button
            type="button"
            onClick={handleClose}
            className="rounded-lg p-1.5 text-[var(--lt-muted)] hover:bg-black/[0.05] hover:text-[var(--lt-text)]"
            aria-label="Fermer"
          >
            <X className="h-4 w-4" />
          </button>
        ...
        <div className="px-4 py-2">
          ...
          <p className="text-[10px] text-[var(--lt-muted)]">
        ...
        <div className="max-h-[min(60vh,420px)] overflow-y-auto px-4 pb-4">
          {step === 0 && (
            <div className="space-y-2">
              <p className="text-[11px] text-[var(--lt-sub)]">Type de document (structure + blocs)</p>
              <select
                value={docType}
                onChange={(e) => setDocType(e.target.value)}
                className="w-full rounded-lg border border-[var(--lt-border)] bg-[var(--lt-card-bg)] px-3 py-2 text-[12px] text-[var(--lt-text)]"
              >
          ...
          {step === 1 && (
            <div className="space-y-2">
              <label className="text-[11px] text-[var(--lt-sub)]">Destinataire</label>
              <input
                value={recipient}
                onChange={(e) => setRecipient(e.target.value)}
                placeholder="Nom, service ou organisation"
                className="w-full rounded-lg border border-[var(--lt-border)] bg-[var(--lt-card-bg)] px-3 py-2 text-[12px] text-[var(--lt-text)] placeholder:text-[#A1A1AA]"
              />
            </div>
          )}
          {step === 2 && (
            <div className="space-y-2">
              <label className="text-[11px] text-[var(--lt-sub)]">Objet</label>
              <input
                value={subject}
          ...
          {step === 3 && (
            <div className="space-y-2">
              <label className="text-[11px] text-[var(--lt-sub)]">Contenu / consigne</label>
              <textarea
                value={content}
          ...
          {step === 4 && (
            <div className="space-y-2">
              <label className="text-[11px] text-[var(--lt-sub)]">Signature (nom / fonction)</label>
              <input
                value={signature}
        ...
        <div className="flex items-center justify-between gap-2 border-t border-[var(--lt-border)] px-4 py-3">
          <button
            ...
            className="inline-flex items-center gap-1 rounded-lg border border-[var(--lt-border)] bg-[var(--lt-card-bg)] px-3 py-2 text-[11px] text-[var(--lt-sub)] hover:bg-[var(--lt-inner-bg)]"
          >
