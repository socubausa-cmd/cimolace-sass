...
export default function TenantPayoutProvidersForm({ initialTenantSlug, lockTenantSlug = false } = {}) {
  ...
  const [pawaToken, setPawaToken] = useState('');
  const [pawaEnv, setPawaEnv] = useState('sandbox');
  ...
  const webhookRows = billingCtx?.webhookUrls
    ? [
        ...
        { key: 'pawapay', label: 'Webhook PawaPay', url: billingCtx.webhookUrls.pawapay ?? `${window.location.origin.replace('5173','4002')}/checkout/webhook/pawapay` },
      ]
    : [];

  const savePawaPay = async () => {
    await onSave('pawapay', {
      api_token: pawaToken,
      environment: pawaEnv,
    }, {});
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── PawaPay — Mobile Money Afrique ─────────────────────────────── */}
      <Card id="payment-card-pawapay" className="bg-[#192734] border-white/10 scroll-mt-24">
        <CardHeader>
          <CardTitle className="text-white flex items-center gap-2">
            <span style={{ background: '#E85D04', borderRadius: '6px', padding: '2px 8px', fontSize: '12px', fontWeight: 800, color: '#fff' }}>PawaPay</span>
            Mobile Money (CMR, CIV, RWA, GHA, SEN…)
          </CardTitle>
          <CardDescription className="text-gray-400">
            PawaPay agrège Orange Money, MTN Money, Airtel Money et d'autres opérateurs.
            Votre token API se trouve dans le tableau de bord PawaPay → Settings → API Keys.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3 max-w-xl">
          <div className="grid gap-2">
            <Label className="text-gray-300">API Token PawaPay *</Label>
            <Input
              type="password"
              placeholder="pp_tk_live_xxxx…"
              value={pawaToken}
              onChange={(e) => setPawaToken(e.target.value)}
              className="bg-[#0F1419] border-white/10 text-white font-mono text-sm"
            />
          </div>
          <div className="grid gap-2">
            ...
            <select
              value={pawaEnv}
              onChange={(e) => setPawaEnv(e.target.value)}
              className="rounded-md border border-white/10 bg-[#0F1419] px-3 py-2 text-sm text-white"
            >
          ...
          <div className="grid gap-2">
            <Label className="text-gray-300">URL Webhook PawaPay</Label>
            <div className="flex gap-2">
              <code className="flex-1 rounded bg-[#0F1419] border border-white/10 px-3 py-2 text-xs text-gray-300 overflow-x-auto">
                {`${window.location.origin.replace(':5173', ':4002')}/checkout/webhook/pawapay`}
              </code>
              <Button size="sm" variant="outline" className="border-white/10 text-gray-400 hover:text-white shrink-0"
                onClick={() => copyText('URL Webhook PawaPay', `${window.location.origin.replace(':5173', ':4002')}/checkout/webhook/pawapay`)}>
                <Copy className="h-3 w-3" />
              </Button>
            </div>
            <p className="text-xs text-gray-500">Coller cette URL dans Dashboard PawaPay → Settings → Callbacks.</p>
          </div>
          <Button
            onClick={savePawaPay}
            disabled={busy === 'pawapay' || !pawaToken.trim() || !billingCtx?.platform?.billingSecretsKeyConfigured}
            className="bg-orange-600 hover:bg-orange-500 text-white"
          >
            {busy === 'pawapay' ? '…' : 'Enregistrer PawaPay'}
          </Button>
        </CardContent>
      </Card>
