  ...
  return (
    <div className="space-y-4">
        ...
        {list.map((p, i) => {
          ...
          return (
            <button
              ...
              aria-selected={isOn}
              onClick={() => onPick(i)}
              className={`relative shrink-0 rounded-full p-0.5 transition-transform duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-[color-mix(in_srgb,var(--school-accent)_70%,transparent)] ${
                isOn ? 'scale-110 ring-2 ring-[color-mix(in_srgb,var(--school-accent)_80%,transparent)]' : 'opacity-75 hover:opacity-100 hover:ring-1 hover:ring-white/20'
              }`}
            >
      ...
      <div className="relative min-h-[5.5rem] rounded-xl border border-white/10 bg-black/35 px-4 py-3 backdrop-blur-md">
        <div className="mb-2 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-[color-mix(in_srgb,var(--school-accent)_90%,transparent)]">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <p className="mt-2 text-xs text-white/55">
              <span className="font-semibold text-[var(--school-accent)]">{current.name}</span>
