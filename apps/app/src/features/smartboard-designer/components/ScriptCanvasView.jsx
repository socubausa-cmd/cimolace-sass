  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center gap-2">
        <Icon className={`h-3.5 w-3.5 ${accent ? 'text-[var(--school-accent,#D4AF37)]' : 'text-white/40'}`} />
          ...
          <ol className="flex flex-col gap-2">
            {keyPoints.map((pt, i) => (
              <li key={i} className="flex items-start gap-2.5">
                <span
                  className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[10px] font-bold text-[var(--school-accent,#D4AF37)]"
