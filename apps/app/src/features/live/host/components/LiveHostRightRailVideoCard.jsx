  ...
  return (
    ...
    >
      ...
      <div style={{display:'flex',gap:'4px',marginTop: showHostRightRailVideoFrame || hostRightRailVideoIsCenterCameraOnly ? '6px' : '4px'}}>
        ...
        <button onClick={() => { if (!guestCamLocked) toggleCamera(); }} title={guestCamLocked ? 'Caméra désactivée par le formateur' : (cameraOn ? 'Couper caméra' : 'Activer caméra')} style={{borderRadius:'8px',border:`1px solid ${cameraOn?'rgba(212,163,106,.5)':'rgba(255,255,255,.09)'}`,background:cameraOn?'rgba(168,118,58,.2)':'rgba(255,255,255,.04)',padding:'4px',color:cameraOn?'#efe0c4':'rgba(255,255,255,.7)',display:'flex',alignItems:'center',cursor:guestCamLocked?'not-allowed':'pointer',opacity:guestCamLocked?0.45:1}}>
        ...
        <button
          ...
          style={{
            borderRadius: '8px',
            border: `1px solid ${sharingScreen ? 'rgba(168,118,58,.5)' : 'rgba(255,255,255,.09)'}`,
            background: sharingScreen ? 'rgba(168,118,58,.18)' : 'rgba(255,255,255,.04)',
            padding: '4px',
            color: sharingScreen ? '#e3c79a' : 'rgba(255,255,255,.7)',
            display: 'flex',
            alignItems: 'center',
