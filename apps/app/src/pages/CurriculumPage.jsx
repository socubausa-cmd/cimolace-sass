  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0F1419] flex items-center justify-center">
        <div className="w-16 h-16 border-4 border-[var(--school-accent)] border-t-transparent rounded-full animate-spin"></div>
      </div>
    );
  }
  ...
  if (!academicYear) {
    return (
      <div className="min-h-screen bg-[#0F1419] flex flex-col items-center justify-center text-white p-4">
        ...
        <Link to="/">
          <Button className="bg-[var(--school-accent)] text-black">Retour à l'accueil</Button>
        </Link>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#0F1419] text-white pt-24 pb-20">
      ...
      <section className="relative px-6 py-16 lg:py-24 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto text-center">
          <div className="inline-block px-4 py-1.5 bg-[color-mix(in_srgb,var(--school-accent)_10%,transparent)] border border-[color-mix(in_srgb,var(--school-accent)_30%,transparent)] rounded-full text-[var(--school-accent)] text-sm font-bold tracking-wide uppercase mb-6">
      ...
      <section className="px-6 py-16 max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {trimesters.map((trimester) => {
            ...
            return (
              <div key={trimester.id} className="group bg-[#192734] border border-white/5 rounded-2xl p-8 hover:border-[color-mix(in_srgb,var(--school-accent)_30%,transparent)] transition-all duration-300 flex flex-col h-full">
                <div className="mb-6">
                  <span className="text-xs font-bold text-[var(--school-accent)] uppercase tracking-wider mb-2 block">Trimestre {trimester.number}</span>
                  <h3 className="text-2xl font-bold text-white mb-3 group-hover:text-[var(--school-accent)] transition-colors">
                ...
                <div className="mt-auto space-y-6">
                  ...
                  <div className="pt-6 border-t border-white/5 flex items-center justify-between">
                     ...
                     <Link to={`/curriculum/trimester/${trimester.id}`}>
                        <Button variant="ghost" className="text-[var(--school-accent)] hover:text-white hover:bg-[color-mix(in_srgb,var(--school-accent)_20%,transparent)] p-0 h-auto font-bold flex gap-2 items-center">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
               {modules.map((module) => (
                  <Link to={`/curriculum/module/${module.id}`} key={module.id} className="block group">
                     <div className="bg-[#0F1419] border border-white/10 rounded-xl p-6 hover:border-[var(--school-accent)] transition-all h-full flex flex-col">
                        <div className="flex justify-between items-start mb-4">
                           <span className="bg-white/5 text-white text-xs font-bold px-3 py-1 rounded border border-white/5 group-hover:bg-[var(--school-accent)] group-hover:text-black transition-colors">
                              {module.code}
                           </span>
                           <span className="text-sm text-gray-500">{module.duration_weeks} sem.</span>
                        </div>
                        <h4 className="text-lg font-bold text-white mb-3 leading-snug group-hover:text-[var(--school-accent)] transition-colors">
                           {module.title}
                        </h4>
