...
const MyFormationsPage = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [{ data: publishedRows, error: pubErr }, { data: enrollmentRows, error: enrErr }, participantsRes] = await Promise.all([
        supabase
          .from('courses')
          .select('id, title, description, status, cycle, duration_weeks, image_url, price_cents, meta')
          .eq('status', 'published')
          .order('created_at', { ascending: false }),
        supabase
          .from('student_progress')
          .select('id, status, created_at, completed_at, course_id, courses(id, title, description, image_url, duration_weeks, meta, price_cents)')
          .eq('user_id', user.id)
          .order('created_at', { ascending: false }),
  ...
  const enrolledByFormationId = useMemo(() => {
    ...
    for (const e of enrollments) {
      if (e?.course_id) map[e.course_id] = e;
    }
    return map;
  }, [enrollments]);
  ...
  const mesCours = useMemo(() => {
    return enrollments
      .map((e) => ({
        ...
        duration_weeks: e.courses?.duration_weeks || null,
