...
const InitiationCeremonyPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0F1419] font-serif pb-20 selection:bg-[color-mix(in_srgb,var(--school-accent)_30%,transparent)]">
      <Helmet>
      ...
      <section className="relative h-[60vh] w-full overflow-hidden flex items-center justify-center">
        ...
        <div className="relative z-10 text-center px-4">
          ...
          >
            <Sparkles className="w-16 h-16 text-[var(--school-accent)] mx-auto mb-6 animate-pulse" />
            <h1 className="text-5xl md:text-7xl font-bold text-white mb-4 tracking-wider">INITIATION</h1>
            <p className="text-xl md:text-2xl text-[var(--school-accent)] tracking-[0.2em] uppercase">Cérémonie de Rentrée Solennelle</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-6 py-16 text-center">
        <p className="text-xl text-gray-300 leading-relaxed italic border-l-4 border-[var(--school-accent)] pl-6 text-left">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-16">
          {values.map((val, idx) => (
            <div key={idx} className="flex flex-col items-center p-4 bg-white/5 rounded-lg border border-white/5 hover:border-[color-mix(in_srgb,var(--school-accent)_50%,transparent)] transition-colors">
              <val.icon className="w-8 h-8 text-[var(--school-accent)] mb-3" />
      ...
      <section className="max-w-5xl mx-auto px-6 py-16">
        ...
        <div className="space-y-12 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-[color-mix(in_srgb,var(--school-accent)_50%,transparent)] before:to-transparent">
          {phases.map((phase, idx) => (
            <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
              
              {/* Icon Marker */}
              <div className="flex items-center justify-center w-10 h-10 rounded-full border-2 border-[var(--school-accent)] bg-[#0F1419] text-[var(--school-accent)] shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 font-bold">
                {phase.step}
              </div>
              
              {/* Content */}
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-6 bg-[#192734] rounded-xl border border-white/5 shadow-lg hover:border-[color-mix(in_srgb,var(--school-accent)_30%,transparent)] transition-all">
                <h3 className="text-xl font-bold text-[var(--school-accent)] mb-2">{phase.title}</h3>
                ...
                {phase.oath && (
                  <div className="bg-black/30 p-3 rounded mt-2 border border-[color-mix(in_srgb,var(--school-accent)_10%,transparent)]">
                    <p className="text-sm text-[var(--school-accent)] font-serif italic text-center">"{phase.oath}"</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-10">
            <div className="p-6 bg-[#0F1419] rounded-xl border border-white/5">
              <Clock className="w-8 h-8 text-[var(--school-accent)] mx-auto mb-4" />
              <h4 className="text-white font-bold">Durée</h4>
              <p className="text-gray-400">60 - 90 minutes</p>
            </div>
            <div className="p-6 bg-[#0F1419] rounded-xl border border-white/5">
              <DollarSign className="w-8 h-8 text-[var(--school-accent)] mx-auto mb-4" />
          ...
          <Button className="bg-[var(--school-accent)] text-black hover:bg-[#b5952f] px-8 py-6 text-lg font-bold rounded-full shadow-[0_0_20px_rgba(212,175,55,0.3)] hover:shadow-[0_0_30px_rgba(212,175,55,0.5)] transition-all">
