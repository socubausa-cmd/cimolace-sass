  ...
  return (
    <div className="min-h-screen bg-[#0F1419] font-sans text-gray-300 selection:bg-[color-mix(in_srgb,var(--school-accent)_30%,transparent)]">
      ...
      <section className="relative pt-32 pb-20 px-4 overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-[color-mix(in_srgb,var(--school-accent)_10%,transparent)] via-[#0F1419] to-[#0F1419]" />
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          ...
          >
            <div className="flex justify-center mb-8">
               <div className="p-4 rounded-full border border-[color-mix(in_srgb,var(--school-accent)_30%,transparent)] bg-[color-mix(in_srgb,var(--school-accent)_5%,transparent)] relative">
                  <div className="absolute inset-0 rounded-full animate-pulse bg-[color-mix(in_srgb,var(--school-accent)_10%,transparent)] blur-xl"></div>
                  <Users className="w-12 h-12 text-[var(--school-accent)] relative z-10" />
               </div>
            </div>
            <h1 className="text-4xl md:text-5xl lg:text-7xl font-serif font-bold text-white mb-6 leading-tight tracking-tight">
              L'ORDRE MYSTIQUE<br/><span className="text-[var(--school-accent)]">DES MANIKONGO</span>
            </h1>
            <div className="flex items-center justify-center gap-4 text-[color-mix(in_srgb,var(--school-accent)_60%,transparent)] my-6">
                <div className="h-px w-20 bg-gradient-to-r from-transparent to-[var(--school-accent)]"></div>
                <span className="text-2xl font-serif">═══</span>
                <div className="h-px w-20 bg-gradient-to-l from-transparent to-[var(--school-accent)]"></div>
            </div>
      ...
      <div className="max-w-6xl mx-auto px-6 sm:px-8 pb-32 relative z-20">
        ...
        >
          ...
          <div className="grid md:grid-cols-2 gap-10">
             <motion.div variants={fadeInUp} className="bg-[#192734]/50 p-8 rounded-2xl border border-white/5 relative overflow-hidden group hover:border-[color-mix(in_srgb,var(--school-accent)_30%,transparent)] transition-colors">
                <div className="absolute top-0 right-0 w-32 h-32 bg-[color-mix(in_srgb,var(--school-accent)_5%,transparent)] rounded-full blur-2xl -mr-10 -mt-10 group-hover:bg-[color-mix(in_srgb,var(--school-accent)_10%,transparent)] transition-all"></div>
                <h3 className="text-2xl font-serif text-white mb-4 flex items-center gap-3">
                   <ScrollText className="w-6 h-6 text-[var(--school-accent)]" />
               ...
               {manikongos.map((m) => (
                  <motion.div 
                     ...
                     className={`p-6 rounded-xl border relative overflow-hidden transition-all duration-300 ${
                        m.isActive 
                           ? 'bg-gradient-to-br from-[color-mix(in_srgb,var(--school-accent)_20%,transparent)] to-[#0F1419] border-[var(--school-accent)] shadow-[0_0_30px_rgba(212,175,55,0.1)]' 
                           : 'bg-[#192734]/30 border-white/5 hover:border-white/20'
                     }`}
                  >
                     <div className="flex justify-between items-start mb-4">
                        <span className={`text-4xl font-serif font-bold opacity-20 ${m.isActive ? 'text-[var(--school-accent)]' : 'text-white'}`}>
                           {m.number}
                        </span>
                        <div className={`p-2 rounded-lg ${m.isActive ? 'bg-[var(--school-accent)] text-black' : 'bg-white/5 text-gray-400'}`}>
                           <m.icon className="w-5 h-5" />
                        </div>
                     </div>
                     <h4 className={`font-serif font-bold text-lg mb-1 ${m.isActive ? 'text-[var(--school-accent)]' : 'text-white'}`}>
                        {m.title}
                     </h4>
                     ...
                     {m.isActive && (
                        <div className="mt-4 pt-4 border-t border-[color-mix(in_srgb,var(--school-accent)_30%,transparent)] flex items-center gap-2 text-xs font-bold text-[var(--school-accent)]">
          ...
          {/* Mandat de l'école */}
          <motion.div variants={fadeInUp} className="bg-gradient-to-r from-[#192734] to-[#0F1419] p-10 rounded-2xl border border-[color-mix(in_srgb,var(--school-accent)_20%,transparent)] shadow-lg">
             <div className="flex flex-col md:flex-row items-center gap-8">
                <div className="flex-shrink-0 p-6 rounded-full bg-[color-mix(in_srgb,var(--school-accent)_10%,transparent)] border border-[color-mix(in_srgb,var(--school-accent)_30%,transparent)]">
                   <Eye className="w-12 h-12 text-[var(--school-accent)]" />
                </div>
                <div>
          ...
          <motion.div variants={fadeInUp} className="text-center pt-10 border-t border-white/5">
            <div className="flex items-center justify-center gap-4 text-[color-mix(in_srgb,var(--school-accent)_40%,transparent)] mb-8">
                <div className="h-px w-24 bg-gradient-to-r from-transparent to-[var(--school-accent)]"></div>
                <span className="text-xl">⸻</span>
                <div className="h-px w-24 bg-gradient-to-l from-transparent to-[var(--school-accent)]"></div>
            </div>
             <p className="font-serif text-xl italic text-gray-400 mb-8">
                "Dans le silence de l'Ordre se forge la parole de la Science."
             </p>
             <Link to="/">
                <Button className="bg-[var(--school-accent)] text-black hover:bg-yellow-500 font-bold px-8">
                  Retour à l'Accueil
                </Button>
             </Link>
          </motion.div>
