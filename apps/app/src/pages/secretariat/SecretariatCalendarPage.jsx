  ...
  return (
    <div className="relative space-y-6">
      <div className="pointer-events-none absolute inset-0 -z-10">
        <div className="absolute top-0 left-1/2 h-[220px] w-[380px] -translate-x-1/2 rounded-full bg-[color-mix(in_srgb,var(--school-accent)_8%,transparent)] blur-[95px]" />
      ...
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div className="space-y-2">
          <h2 className="text-2xl font-bold text-white flex items-center gap-2">
            <Calendar className="w-6 h-6 text-[var(--school-accent)]" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        <motion.div whileHover={{ y: -2, scale: 1.01 }} className="rounded-xl border border-[color-mix(in_srgb,var(--school-accent)_25%,transparent)] bg-[color-mix(in_srgb,var(--school-accent)_10%,transparent)] px-4 py-3 shadow-[0_10px_30px_rgba(212,175,55,0.12)]">
          <p className="text-xs text-[var(--school-accent)] uppercase tracking-wider">Demandes en attente</p>
      ...
      {todayEvents.length > 0 && (
        <div className="rounded-2xl border border-[color-mix(in_srgb,var(--school-accent)_30%,transparent)] bg-[color-mix(in_srgb,var(--school-accent)_5%,transparent)] p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {todayEvents.map((e) => (
              <motion.div key={e.id} whileHover={{ y: -1 }} className="flex items-center gap-2 bg-[#0F1419]/50 rounded-lg px-3 py-2 border border-white/10">
                ...
                {e.type === 'live' ? (
                  <Button size="sm" variant="ghost" className="h-7 px-2 text-[var(--school-accent)]" asChild>
                    <Link to={`/live/${e.id}`}>
                      <Video className="w-3 h-3" />
                    </Link>
                  </Button>
                ) : e.video_meeting_url ? (
                  <Button size="sm" variant="ghost" className="h-7 px-2 text-[var(--school-accent)]" onClick={() => window.open(e.video_meeting_url, '_blank')}>
                    <Video className="w-3 h-3" />
                  </Button>
                ) : null}
              </motion.div>
            ...
            {pendingRequests.map((r) => (
              ...
              >
                ...
                <div className="flex gap-2">
                  ...
                  <Button size="sm" className="bg-[var(--school-accent)] text-black hover:bg-amber-500 hover:shadow-[0_0_20px_rgba(212,175,55,0.3)] transition-all" onClick={() => setConfirmModal(r)}>
                    Confirmer
                  </Button>
                </div>
              </motion.div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <span className={cn(
                  ...
                  eventModal.type === 'appointment' && 'bg-[color-mix(in_srgb,var(--school-accent)_20%,transparent)] text-[var(--school-accent)]',
              ...
              {eventModal.student && <p className="text-sm text-[var(--school-accent)] flex items-center gap-1"><User className="w-4 h-4" /> {eventModal.student.name || eventModal.student.email}</p>}
              {eventModal.teacher && !eventModal.student && <p className="text-sm text-[var(--school-accent)] flex items-center gap-1"><User className="w-4 h-4" /> {eventModal.teacher.name}</p>}
              <div className="flex gap-2 pt-2">
                ...
                {eventModal.video_meeting_url && eventModal.type !== 'live' && (
                  <Button variant="outline" className="border-[color-mix(in_srgb,var(--school-accent)_30%,transparent)] text-[var(--school-accent)]" onClick={() => window.open(eventModal.video_meeting_url, '_blank')}>
