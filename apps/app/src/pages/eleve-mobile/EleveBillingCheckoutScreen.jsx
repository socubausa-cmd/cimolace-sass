 ...
 * le contenu (le shell fournit déjà `EleveImmersiveHalo` ; ne pas le dupliquer ni masquer avec `!bg` sur `main`).
 * Route : `/m/eleve/billing/checkout/:id`
 */
export default function EleveBillingCheckoutScreen() {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div className="flex min-w-0 justify-start">
            <button
              type="button"
              onClick={() => navigate(-1)}
              className="flex min-w-0 items-center gap-1 rounded-xl py-2 pl-0 pr-2 text-[13px] text-white/55 transition hover:bg-white/5 hover:text-white"
            >
          ...
          <div className="flex justify-center">
            <div className="flex max-w-full items-center gap-1.5 rounded-full border border-white/[0.1] bg-white/[0.04] px-2.5 py-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]">
              ...
              <span className="truncate text-[10px] font-bold uppercase tracking-[0.2em] text-white/75">Paiement</span>
            </div>
          </div>
        ...
        <div className="mx-auto w-full max-w-lg min-h-0 flex-1 overflow-y-auto px-4 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-3">
          <BillingCheckoutView variant="eleve" />
        </div>
      </div>
    </EleveMobileShell>
  );
