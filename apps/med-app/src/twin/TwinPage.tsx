...
export function TwinPage() {
  ...
  if (loading) {
    return <div style={{ padding: 40, textAlign: 'center', color: 'var(--zw-text-faint)' }}>{t('common:loading')}</div>;
  }
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <h2 style={{ fontSize: 23, fontWeight: 800, margin: '4px 0 0', color: 'var(--zw-text)', display: 'flex', alignItems: 'center', gap: 9 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button
            ...
            style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 36, height: 36, background: '#fff', color: 'var(--brand-primary)', border: '1px solid var(--zw-border)', borderRadius: 10, cursor: 'pointer' }}
          >
            <HelpCircle size={16} />
          </button>
          <button
      ...
      <div className="twin-tabs" style={{ display: 'flex', gap: 4, marginBottom: 16, flexWrap: 'wrap', background: 'var(--zw-bg-subtle)', padding: 4, borderRadius: 10 }}>
        {TABS.map((tabItem) => {
          const tourId = tabItem.key === 'body' ? 'corps-3d' : tabItem.key === 'lab' ? 'laboratoire' : tabItem.key === 'copilot' ? 'copilote' : undefined;
          return (
            <button key={tabItem.key} className="twin-tab" data-tour-id={tourId} onClick={() => setTab(tabItem.key)} style={{ padding: '7px 14px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, background: tab === tabItem.key ? '#fff' : 'transparent', color: tab === tabItem.key ? 'var(--brand-primary)' : 'var(--zw-text-muted)', boxShadow: tab === tabItem.key ? '0 1px 3px rgba(15,23,42,0.12)' : 'none', whiteSpace: 'nowrap' }}>
              {t(tabItem.i18nKey)}
            </button>
          );
        })}
        ...
        <div style={panel}>
          {!selectedOrgan ? (
            <div>
              ...
              {scoredCount === 0 ? (
                <p style={{ fontSize: 13, color: 'var(--zw-text-muted)' }}>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 14 }}>
                {(['green', 'yellow', 'orange', 'red'] as OrganColor[]).map((c) => (
                  <div key={c} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--zw-text-soft)' }}>
              ...
              <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 6 }}>
                {organs.filter((o: any) => o.score).sort((a: any, b: any) => a.score.score - b.score.score).map((o: any) => (
                  <button key={o.code} onClick={() => setSelected(o.code)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', background: 'var(--zw-bg)', border: 'none', borderRadius: 8, cursor: 'pointer', textAlign: 'left' }}>
      ...
      <div className="twin-grid-3col" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 16 }}>
        ...
        <div style={panel}>
          ...
          {(state?.alerts || []).length === 0 ? (
            <p style={{ fontSize: 13, color: 'var(--zw-text-faint)' }}>Aucune alerte.</p>
          ) : (
            (state.alerts).map((a: any) => (
              <div key={a.id} style={{ padding: 10, borderRadius: 8, marginBottom: 8, background: a.severity === 'critical' ? '#fef2f2' : a.severity === 'warning' ? '#fffbeb' : '#f0f9ff', borderLeft: `3px solid ${a.severity === 'critical' ? '#ef4444' : a.severity === 'warning' ? '#f59e0b' : '#0ea5e9'}` }}>
                <div style={{ fontSize: 12.5, color: 'var(--zw-text-soft)' }}>{a.message_fr}</div>
              </div>
            ))
          )}
        </div>
        ...
        <div style={panel}>
          ...
          <div className="twin-lab-row" style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
            <select value={pickCode} onChange={(e) => setPickCode(e.target.value)} style={{ flex: 1, padding: '7px 8px', border: '1px solid var(--zw-border)', borderRadius: 7, fontSize: 12.5, minWidth: 0 }}>
              <option value="">— Biomarqueur —</option>
              {(refs.biomarkers || []).map((b: any) => <option key={b.code} value={b.code}>{b.name_fr} ({b.unit})</option>)}
            </select>
            <input type="number" placeholder="val." value={pickValue} onChange={(e) => setPickValue(e.target.value)} style={{ width: 70, padding: '7px 8px', border: '1px solid var(--zw-border)', borderRadius: 7, fontSize: 12.5 }} />
            <button
          ...
          {staged.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              {staged.map((s) => (
                <div key={s.biomarker_code} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, padding: '3px 6px', color: 'var(--zw-text-soft)' }}>
          ...
          <div style={{ maxHeight: 180, overflowY: 'auto' }}>
            {(state?.biomarkers || []).length === 0 ? (
              <p style={{ fontSize: 12, color: 'var(--zw-text-faint)' }}>Aucune valeur enregistrée.</p>
            ) : (
              (state.biomarkers).map((b: any) => {
                ...
                return (
                  <div key={b.biomarker_code} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12, padding: '4px 0', borderBottom: '1px solid var(--zw-bg-subtle)' }}>
                    <span style={{ color: 'var(--zw-text-soft)' }}>{ref?.name_fr || b.biomarker_code}</span>
                    <span><strong style={{ color: 'var(--zw-text)' }}>{b.value}</strong> <span style={{ color: 'var(--zw-text-faint)' }}>{b.unit_raw || ref?.unit}</span> <span style={{ display: 'inline-block', width: 8, height: 8, borderRadius: '50%', background: fc, marginLeft: 4 }} /></span>
                  </div>
                );
              })
            )}
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--zw-text-faint)' }}><X size={16} /></button>
      </div>
      {!score ? (
        <p style={{ fontSize: 13, color: 'var(--zw-text-faint)' }}>Pas encore de données pour cet organe.</p>
      ) : (
        <>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 12, flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: 13, color: 'var(--zw-text-muted)' }}>/100 · {COLOR_LABEL[score.color as OrganColor]}</span>
          </div>
            ...
            <div style={{ marginBottom: 12 }}>
              {Object.entries(score.dimensions).map(([dim, v]: any) => (
                <div key={dim} style={{ marginBottom: 5 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--zw-text-muted)' }}><span>{dim}</span><span>{v}</span></div>
                  <div style={{ height: 5, background: 'var(--zw-bg-subtle)', borderRadius: 3 }}><div style={{ width: `${v}%`, height: '100%', background: COLOR_HEX[(v >= 80 ? 'green' : v >= 60 ? 'yellow' : v >= 40 ? 'orange' : 'red') as OrganColor], borderRadius: 3 }} /></div>
                </div>
              ))}
            </div>
          )}
          ...
          {assistant && (
            <div style={{ marginTop: 12, padding: 12, background: '#faf5ff', borderRadius: 10, border: '1px solid #e9d5ff' }}>
              <div style={{ fontSize: 12.5, color: 'var(--zw-text)', lineHeight: 1.5 }}>{assistant.explanation_fr}</div>
              ...
              {assistant.confidence != null && (
                <div style={{ marginTop: 6, fontSize: 11, color: 'var(--zw-text-faint)' }}>Confiance : {Math.round(assistant.confidence * 100)}%</div>
              )}
            </div>
          )}
        </>
