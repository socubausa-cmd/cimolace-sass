...
export class MasterclassFactoryController {
  constructor(private svc: MasterclassFactoryService, private usage: UsageService) {}
  @Post('generate') @Roles('owner','admin','teacher') async generate(@Body() d: any, @CurrentTenant() t: TenantContext, @Req() r: Request) { await this.usage.assertAiCredit(t.id, 'ai:masterclass:generate'); return this.svc.generateFromText(t.id, (r as any).user.id, d.title, d.sourceText); }
  ...
  @Post('analyze') @Roles('owner','admin','teacher') async analyzeDoc(@Body() d: any, @CurrentTenant() t: TenantContext) { await this.usage.assertAiCredit(t.id, 'ai:masterclass:analyze'); return this.svc.analyzeDocument(t.id, d.url); }
}
