...
export function AttachmentsPanel({
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr auto', gap: 8, marginBottom: 12 }}>
        ...
        <label
          style={{
            ...
            background: uploading ? '#b0aaa2' : '#3b82f6', color: '#fff',
      ...
      {items.length === 0 ? (
        <p style={{ color: '#b0aaa2', fontSize: 13, textAlign: 'center', padding: 20, background: '#fafaf8', borderRadius: 8 }}>
        ...
        <ul style={{ paddingLeft: 0, listStyle: 'none', margin: 0 }}>
          {items.map((att) => {
            ...
            return (
              <li
                key={att.id}
                style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: '#fafaf8', borderRadius: 8, marginBottom: 4 }}
              >
                <Icon size={18} color="#475569" style={{ flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 500, color: '#1e1e1e', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {att.file_name}
                  </div>
                ...
                {canTogglePatientVisibility && (
                  <button
                    ...
                    style={{ background: 'none', border: '1px solid #ece7e1', borderRadius: 4, padding: 4, cursor: 'pointer', color: att.visible_to_patient ? 'var(--brand-accent)' : '#b0aaa2' }}
                  >
                ...
                <button
                  ...
                  title="Telecharger"
                  style={{ background: 'none', border: '1px solid #ece7e1', borderRadius: 4, padding: 4, cursor: 'pointer', color: '#3b82f6' }}
                >
                  <Download size={14} />
                </button>
                <button
                  ...
                  title="Supprimer"
                  style={{ background: 'none', border: '1px solid #ece7e1', borderRadius: 4, padding: 4, cursor: 'pointer', color: '#dc2626' }}
                >
                  <Trash2 size={14} />
                </button>
              </li>
