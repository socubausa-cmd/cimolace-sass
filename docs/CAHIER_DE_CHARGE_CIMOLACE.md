...
- **Scope moteur par préfixe de clé ✅ (observe-mode, commit `b6194832`, test 4/4)** : `auth/ApiKeyGuard` (garde `/v1/mbolo/*` + `/v1/medos/*`) détecte une clé `mbk_`/`mdk_` utilisée sur un AUTRE moteur (least-privilege ; `cml_`=wildcard). Déployé en **warn (0 régression)** ; `API_KEY_SCOPE_ENFORCE=1` (Railway) → 403 dur une fois les logs confirmés propres.
...
**MàJ 2026-07-03 (soir) — tout DÉPLOYÉ + branche débloquée** :
- `/sdk/cimolace-sdk.js` **servi en vrai JS** (front déployé). SDK des 3 moteurs live.
- **Branche HEAD re-buildable** (commit `310484f6`, 3 fichiers untracked référencés par HEAD committés) → **front déployé PROPREMENT depuis HEAD canonique** (`target=production ● Ready`, aliasé prorascience.org/app/liri) ; CI débloquée.
- mbolo embed E2E prouvé par pièces (page rend standalone + API 200 catalogue leak-free + CORS app.cimolace.space) — capture visuelle du rendu live impossible (preview bound localhost ; dev CORS-bloqué).

**✅ E2E « WRITE » COMPLET EXÉCUTÉ + PROUVÉ EN PROD (2026-07-04)** — USER a fourni un compte existant (socubausa@gmail.com) → provisioning service-role (PAS de création de compte ni de mot de passe) :
1. **Org créée** : tenant `demo-e2e-cimolace` (« Démo E2E — Boutique Cimolace », owner = socubausa, id `690b209e…`) + membership owner. ✅
2. **Moteur activé** : `tenant_services.mbolo = active`. ✅
3. **Catalogue rempli** : catégorie « Boutique » + 2 produits (Coffret Bien-être 25 000 FCFA, Huile premium 12 000 FCFA). ✅
4. **Embarqué / intégré** : `GET /v1/mbolo/embed/demo-e2e-cimolace/catalog` → **200** live ; iframe `/embed/boutique?tenant=demo-e2e-cimolace` **rendue pristine** (capture) avec les 2 produits, CORS `app.cimolace.space` OK. ✅

→ Parcours client « activer un moteur → créer une organisation → embarquer/héberger/intégrer » **démontré de bout en bout en production**. (Org de démo conservée ; supprimable via service-role si besoin.) Reste optionnel : E2E avec un VRAI paiement (mouvement d'argent) + nettoyage données démo.
