...
export class AiWorkerService {
  ...
  // ── Bloc 11 — Workers (Renewal, DLQ, Expiry) ────────────────────────────

  async processRenewalCycle() {
    const subs = await (this.supabase.client as any).from('billing_subscriptions').select('*').eq('status', 'active').lte('current_period_end', new Date(Date.now() + 3 * 86400000).toISOString());
    return { processed: (subs.data || []).length };
  }

  async processDlq() {
    const events = await (this.supabase.client as any).from('billing_events').select('*').eq('processed', false).limit(10);
    for (const e of (events.data || [])) {
      await (this.supabase.client as any).from('billing_events').update({ retry_count: 1 }).eq('id', e.id);
    }
    return { retried: (events.data || []).length };
  }

  async processExpiredSubscriptions() {
    const expired = await (this.supabase.client as any).from('billing_subscriptions').select('*').eq('status', 'active').lte('current_period_end', new Date().toISOString());
    for (const sub of (expired.data || [])) {
      await (this.supabase.client as any).from('billing_subscriptions').update({ status: 'expired' }).eq('id', sub.id);
    }
    return { expired: (expired.data || []).length };
  }
