...
export class CinetPayProvider implements PaymentProvider {
  ...
  async parseWebhookPayload(event: WebhookEvent): Promise<PaymentConfirmation | null> {
    try {
      ...
      return {
        ...
        provider: 'cinetpay',
        status,
        amountCents: Math.round(Number(payload.amount || payload.cpm_amount || 0)),
        currency: (payload.currency || payload.cpm_currency || 'XOF').toUpperCase(),
        customerEmail: payload.customer_email || null,
        customerPhone: payload.customer_phone_number || payload.cel_phone_num || null,
  ...
  async getSubscription(_id: string): Promise<{ status: SubscriptionStatus; currentPeriodEnd?: string }> {
  ...
  private mapStatus(status: string): PaymentConfirmation['status'] {
    ...
    if (['success', 'succeeded', 'completed', 'accepted', '00', 'successful'].includes(s)) return 'succeeded';
    if (['failed', 'cancelled', 'canceled', 'refused', 'error'].includes(s)) return 'failed';
    return 'pending';
  }
}
