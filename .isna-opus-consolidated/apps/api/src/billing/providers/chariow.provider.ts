...
export class ChariowProvider implements PaymentProvider {
  ...
  async verifyWebhook(event: WebhookEvent): Promise<boolean> {
    ...
    const expected = createHmac('sha256', this.webhookSecret)
      .update(typeof event.rawBody === 'string' ? event.rawBody : event.rawBody.toString())
      .digest('hex');

    const cleanSig = event.signature.replace(/^sha256=/i, '').trim();
    try {
      return createHmac('sha256', this.webhookSecret).update(expected).digest('hex') ===
  ...
  async parseWebhookPayload(event: WebhookEvent): Promise<PaymentConfirmation | null> {
    try {
      const payload = typeof event.rawBody === 'string'
        ? JSON.parse(event.rawBody)
        : JSON.parse(event.rawBody.toString());

      const data = payload.data || payload;
      const sale = data.sale || data;
      const status = this.mapStatus(sale?.status || data?.status || payload?.status);
      ...
      return {
        ...
        customerPhone: sale?.customer?.phone?.number || data?.customer?.phone || null,
  ...
  async getSubscription(_providerSubscriptionId: string): Promise<{ status: SubscriptionStatus; currentPeriodEnd?: string }> {
  ...
  private mapStatus(status: string): PaymentConfirmation['status'] {
    ...
    if (['completed', 'success', 'succeeded', 'paid', 'confirmed'].includes(s)) return 'succeeded';
    if (['failed', 'cancelled', 'canceled', 'expired', 'refused'].includes(s)) return 'failed';
    return 'pending';
  }
}
