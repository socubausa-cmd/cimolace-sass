...
import type { CheckoutResult, CreateCheckoutInput, PaymentConfirmation, PaymentProvider, SubscriptionStatus, WebhookEvent } from '../payment-provider.interface';
...
export class NowPaymentsProvider implements PaymentProvider {
  ...
  async createCheckout(input: CreateCheckoutInput): Promise<CheckoutResult> {
    ...
    return { checkoutUrl: data.invoice_url, sessionId: data.id, providerSessionId: data.id };
  }

  async verifyWebhook(event: WebhookEvent): Promise<boolean> {
    if (!this.webhookSecret) return true;
    const expected = createHmac('sha512', this.webhookSecret).update(typeof event.rawBody === 'string' ? event.rawBody : event.rawBody.toString()).digest('hex');
    return expected === (event.headers['x-nowpayments-sig'] || '');
  }

  async parseWebhookPayload(event: WebhookEvent): Promise<PaymentConfirmation | null> {
    try {
      const p = typeof event.rawBody === 'string' ? JSON.parse(event.rawBody) : JSON.parse(event.rawBody.toString());
      return {
        eventId: event.id, provider: 'nowpayments',
        status: p.payment_status === 'finished' ? 'succeeded' : p.payment_status === 'failed' ? 'failed' : 'pending',
  ...
  async getSubscription(_id: string): Promise<{ status: SubscriptionStatus }> { return { status: 'active' }; }
...
export class PayPalProvider implements PaymentProvider {
  ...
  async createCheckout(input: CreateCheckoutInput): Promise<CheckoutResult> {
    ...
    const res = await fetch(`${this.baseUrl}/v2/checkout/orders`, {
      ...
      body: JSON.stringify({
        intent: 'CAPTURE',
        purchase_units: [{ amount: { currency_code: (input.currency || 'USD').toUpperCase(), value: String(input.priceCents / 100) } }],
  ...
  async verifyWebhook(_event: WebhookEvent): Promise<boolean> { return true; }

  async parseWebhookPayload(event: WebhookEvent): Promise<PaymentConfirmation | null> {
    try {
      const p = typeof event.rawBody === 'string' ? JSON.parse(event.rawBody) : JSON.parse(event.rawBody.toString());
      return {
        eventId: event.id, provider: 'paypal',
