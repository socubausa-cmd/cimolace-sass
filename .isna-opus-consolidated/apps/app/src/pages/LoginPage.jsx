...
const SCHOOL_BRAND = isnaTenantConfig.branding.name;
const SCHOOL_ACADEMY_TITLE = `${SCHOOL_BRAND} Academy`;
...
const LoginPage = () => {
  ...
  if (isLiriMobileAuth && shouldShowStudentInstallGate()) {
    return (
      <EleveConnectionLayout className="text-white">
        <Helmet>
          <title>Installer LIRI | Élève</title>
        </Helmet>
        ...
        <p className={isLiriMobileAuth ? 'mt-1.5 text-[14px] leading-relaxed' : 'text-sm text-gray-400'} style={isLiriMobileAuth ? { color: EV_MUTED } : undefined}>
          {isLiriMobileAuth ? (
            ...
            <>
              ...
              site public ({SCHOOL_BRAND}), c&apos;est la vitrine — pas l&apos;application.
            </>
          )}
        </p>
      </div>
        ...
        <div className="mx-auto w-full max-w-md px-4 pb-8 pt-1">
          ...
          <div className="mb-6 text-center">
            <Link to={ELEVE_MOBILE.home} className="inline-flex flex-col items-center gap-2">
              ...
              />
              <span className="font-sans text-[24px] font-extrabold tracking-tight text-white">{SCHOOL_BRAND}</span>
              <span
  ...
  return (
    <div className="flex min-h-screen bg-[#0F1419]">
      <Helmet>
        <title>{`Connexion | ${SCHOOL_ACADEMY_TITLE}`}</title>
      </Helmet>

      {/* ── PANNEAU GAUCHE – branding ── */}
      <div className="relative hidden w-1/2 flex-col justify-between overflow-hidden p-12 lg:flex">
        <div className="absolute inset-0 bg-gradient-to-br from-[#0F1419] via-[#192734] to-[#0F1419]" />
        <div className="absolute left-1/4 top-1/4 h-[500px] w-[500px] rounded-full bg-[#D4AF37]/5 blur-[150px]" />
        ...
        <div className="relative z-10">
          <Link to="/">
            <span className="font-serif text-2xl font-bold tracking-wider text-white">{SCHOOL_BRAND}</span>
            <span className="mt-0.5 block text-[0.65rem] uppercase tracking-[0.4em] text-[#D4AF37]">Academy</span>
          </Link>
        </div>
        ...
        <div className="relative z-10 flex flex-col items-center">
          <div className="relative">
            <div className="absolute inset-0 scale-110 rounded-full bg-[#D4AF37]/20 blur-2xl" />
            <img
              src="/logo.png"
              alt={SCHOOL_BRAND}
              className="relative h-36 w-36 rounded-full border-2 border-[#D4AF37]/50 shadow-2xl"
            />
          </div>
        </div>
        ...
        <div className="relative z-10 space-y-8">
          <div className="space-y-2">
            <div className="mb-4 flex items-center gap-2">
              ...
              <span className="text-xs font-semibold uppercase tracking-widest text-[#D4AF37]">Citation du jour</span>
            </div>
          ...
          <div className="flex gap-8 border-t border-white/5 pt-4">
            {STATS.map(({ icon: Icon, value, label }) => (
              <div key={label} className="flex flex-col items-center gap-1">
                <Icon className="mb-1 h-5 w-5 text-[#D4AF37]" />
        ...
        {/* Pied de page */}
        <div className="relative z-10">
          <p className="text-xs text-gray-600">© {new Date().getFullYear()} {SCHOOL_BRAND} · NGOWAZULU</p>
        </div>
      </div>
        ...
        >
          ...
          <div className="mb-8 flex flex-col items-center text-center lg:hidden">
            <Link to="/" className="inline-flex flex-col items-center gap-2">
              <img src="/logo.png" alt={SCHOOL_BRAND} className="h-16 w-16 rounded-full border border-[#D4AF37]/30" />
              <span className="font-serif text-2xl font-bold tracking-wider text-white">{SCHOOL_BRAND}</span>
              <span className="text-[0.6rem] uppercase tracking-[0.3em] text-[#D4AF37]">Academy</span>
            </Link>
          </div>
