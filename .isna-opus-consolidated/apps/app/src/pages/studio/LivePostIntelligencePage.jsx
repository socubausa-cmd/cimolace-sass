        ...
        <div className="flex items-center gap-3">
          {rec.public_url && (
            <a
              href={rec.public_url}
              download
              className="h-8 px-4 rounded-xl bg-[#D4AF37]/15 border border-[#D4AF37]/30 text-[#D4AF37] text-xs hover:bg-[#D4AF37]/25 flex items-center gap-1.5 transition-colors"
            >
  ...
  return (
    <div className="relative">
      <textarea
        ...
        rows={6}
        className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm text-white/80 placeholder:text-white/25 outline-none focus:border-[#D4AF37]/40 resize-none transition-colors"
      />
    ...
    <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
      {messages.map((m) => (
        <div key={m.id} className="flex gap-2.5 text-sm">
          <div className="w-6 h-6 rounded-full bg-[#D4AF37]/20 border border-[#D4AF37]/30 flex items-center justify-center flex-shrink-0 text-[10px] font-bold text-[#D4AF37]">
  ...
  return (
    <div className="flex flex-wrap gap-2 pt-1">
      <button
        ...
        className="h-9 px-4 rounded-xl bg-[#D4AF37]/15 border border-[#D4AF37]/30 text-[#D4AF37] text-xs hover:bg-[#D4AF37]/25 flex items-center gap-1.5 transition-colors"
      >
            ...
            <>
              ...
              {row?.postproduction_content_id && (
                <Link
                  ...
                  className="inline-flex items-center gap-1.5 h-8 px-3 rounded-lg bg-[#D4AF37]/15 border border-[#D4AF37]/35 text-[#D4AF37] text-xs hover:bg-[#D4AF37]/25 transition-colors"
                >
          ...
          >
            ...
            <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 pb-[max(1.25rem,env(safe-area-inset-bottom))] space-y-4 pt-3">
              ...
              <button
                ...
                className="min-h-11 w-full rounded-xl border border-[#D4AF37]/35 bg-[#D4AF37]/12 text-sm font-medium text-[#D4AF37]"
              >
  ...
  if (compact && typeof onOpenSheet === 'function') {
    return (
      ...
      >
        ...
        <span className="inline-flex items-center text-[11px] font-medium text-[#D4AF37]">
          Ouvrir la fiche
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3 space-y-2">
      ...
      <button
        type="button"
        onClick={() => setShow((s) => !s)}
        className="text-[11px] text-[#D4AF37] hover:underline min-h-10 sm:min-h-0 py-2 sm:py-0 text-left w-full sm:w-auto"
      >
...
function NeuroRecallReportRow({ title, nodeKey, content, compact = false, timestamp = null, replayUrl = null, onOpenSheet }) {
  ...
  const timestampBadge = timestamp != null ? (
    <a
      ...
      className={cn(
        ...
        'bg-[#D4AF37]/10 text-[#D4AF37]/80 border border-[#D4AF37]/20 shrink-0',
        replayUrl && 'hover:bg-[#D4AF37]/20 cursor-pointer',
      )}
    >
  ...
  if (compact && typeof onOpenSheet === 'function') {
    return (
      ...
      >
        <ChevronRight className="w-4 h-4 text-[#D4AF37]/80 shrink-0 mt-0.5" />
        <div className="min-w-0 flex-1">
          <span className="text-sm text-white/85 line-clamp-2 block">{title || nodeKey}</span>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-[10px] text-[#D4AF37]/90">Lire le rapport</span>
            {timestampBadge}
          </div>
        </div>
      </button>
    ...
    return (
      <Section title="Révision NeuroRecall" icon={GraduationCap}>
        <div className="flex justify-center py-6">
          <Loader2 className="w-5 h-5 text-[#D4AF37]/60 animate-spin" />
        </div>
      </Section>
    );
  }
      ...
      {userId && cards.length > 0 ? (
        <div className="mb-4 rounded-xl border border-white/[0.08] bg-white/[0.02] p-3 space-y-2">
          ...
          <button
            ...
            className={cn(
              'rounded-lg border border-[#D4AF37]/40 bg-[#D4AF37]/10 text-[#D4AF37] hover:bg-[#D4AF37]/20 disabled:opacity-40 disabled:pointer-events-none transition-colors text-[11px]',
    ...
    <div style={{ marginLeft: indent }}>
      <button
        ...
        className={cn(
          ...
          isRoot
            ? 'bg-[#D4AF37]/15 border border-[#D4AF37]/30 text-[#D4AF37] font-semibold text-sm mb-2'
            : depth === 1
...
export default function LivePostIntelligencePage({ mobileLiriShell = false } = {}) {
  ...
  return (
    <div className={cn('text-white', mobileLiriShell ? 'min-h-0' : 'min-h-screen bg-[#05070c]')}>

      {/* Header sticky */}
      <div
        className={cn(
          'border-b border-white/[0.06] backdrop-blur-xl sticky top-0 z-20',
          mobileLiriShell ? 'bg-[#0d0b09]/92 -mx-4 px-4' : 'bg-[#080c14]/90',
        )}
      >
        <div className="max-w-4xl mx-auto py-3 sm:py-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <Link
              ...
              className="flex items-center gap-1.5 text-sm text-white/50 hover:text-[#D4AF37] transition-colors shrink-0"
            >
      ...
      <div className={cn('max-w-4xl mx-auto space-y-4 sm:space-y-5', mobileLiriShell ? 'px-0 py-4' : 'px-4 py-8')}>
        ...
        {/* ── Résumé IA ─────────────────────────────────────────────────────── */}
        <div className="rounded-2xl border border-[#D4AF37]/20 bg-gradient-to-br from-[#D4AF37]/5 to-transparent overflow-hidden">
          <button
            type="button"
          ...
          >
            <div className="w-9 h-9 rounded-xl bg-[#D4AF37]/15 border border-[#D4AF37]/25 flex items-center justify-center flex-shrink-0">
              {aiStatus === 'loading'
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {aiStatus === 'idle' && (
                <span className="h-6 px-2.5 rounded-full bg-[#D4AF37]/20 border border-[#D4AF37]/30 text-[10px] font-semibold text-[#D4AF37]">
                  Générer
                </span>
              )}
              {aiStatus === 'done' && (
