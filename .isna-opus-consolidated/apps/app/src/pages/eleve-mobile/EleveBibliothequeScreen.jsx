...
function buildCourseRowsFromEnrollments(enrollments) {
  return enrollments
    .filter((e) => e?.formations && e.status !== 'completed')
    .map((e) => {
      ...
      const percent = 28 + (String(e.id || '').charCodeAt(0) % 50);
      return {
        id: e.id,
...
export default function EleveBibliothequeScreen() {
  const { user } = useAuth();
  const { enrollments, loading, progressLabel } = useLiriMobileEnrollmentPreview(user?.id);
  const [tab, setTab] = useState('mes');

  const inProgressCount = useMemo(
    () => enrollments.filter((e) => e?.status && e.status !== 'completed' && e.status !== 'dropped').length,
    [enrollments],
  );
  ...
  const globalPct = useMemo(() => {
    ...
    return Math.min(
      100,
      Math.round(
        (inProgressCount * 72 + completedCount * 100 + (total - inProgressCount - completedCount) * 45) / Math.max(1, total),
      ),
    );
      ...
      >
        ...
        <AnimatePresence mode="wait">
          {tab === 'mes' && (
            ...
            >
              <GlobalProgressCard
                globalPct={total ? globalPct : 68}
                coursEnCours={total ? inProgressCount : 12}
                loading={loading}
              />
              {total ? null : (
                <p className="mb-3 -mt-1 text-center text-[10px] text-white/35">Exemples visuels — connecte un compte pour tes données</p>
              )}
