...
const ACHIEVEMENTS = [
  ...
  },
  {
    id: '2',
    tone: 'emerald',
    top: <BookOpen className="h-4 w-4 text-white" strokeWidth={2.2} />,
    subIcon: null,
    title: 'Apprenti assidu',
    sub: '10 cours terminés',
    xp: '20 XP',
  ...
  },
  {
    id: '3',
    tone: 'blue',
    top: <Target className="h-4 w-4 text-white" strokeWidth={2.2} />,
    subIcon: null,
    title: 'Focus master',
    sub: '5 lives suivis',
    xp: '15 XP',
...
export default function EleveProfileScreen() {
  ...
  const schoolName = user?.user_metadata?.school || 'Lycée de la Réussite';
  const classLabel = user?.user_metadata?.class || 'Terminale S';
  ...
  const suivisN = total;
  const terminésN = completedN;

  const showDemo = !user || (!enrollLoading && total === 0);
  const statSuivis = showDemo ? 32 : suivisN;
  const statTerminés = showDemo ? 18 : terminésN;
  const statStreak = 48;
  const statXp = 850;
  ...
  const globalPct = useMemo(() => {
    if (showDemo) return 68;
    if (chTotal <= 0) return 0;
    return Math.min(100, Math.round((chDone / chTotal) * 100));
  }, [showDemo, chDone, chTotal]);
