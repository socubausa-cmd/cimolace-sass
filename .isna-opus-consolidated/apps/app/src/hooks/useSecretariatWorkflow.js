...
export function useSecretariatWorkflow() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [
        ...
        supabase
          .from('enrollments')
          .select('id,student_id,formation_id,status,enrolled_at,pipeline_stage,last_activity_at,assigned_teacher_id')
        ...
        supabase.from('formations').select('id,title,status'),
        supabase.from('attendance_records').select('id,student_id,status,created_at').eq('status', 'absent').is('deleted_at', null),
        supabase
      ...
      const enrollments = isMissingRelationError(enrollmentsRes.error) ? [] : (enrollmentsRes.data || []);
      ...
      const formations = isMissingRelationError(formationsRes.error) ? [] : (formationsRes.data || []);
  ...
  useEffect(() => {
    const channels = [];
    const tableList = ['enrollments', 'billing_subscriptions', 'attendance_records', 'secretariat_alerts'];
