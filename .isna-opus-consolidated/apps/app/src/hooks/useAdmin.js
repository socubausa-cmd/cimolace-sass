...
const buildFallbackActivity = async (supabase, limit = 100) => {
  const [paymentsRes, webhooksRes, formationsRes, enrollmentsRes, profilesRes] = await Promise.all([
    safeRows(
      supabase
        .from('billing_payments')
        .select('id,user_id,payment_status,provider,purchase_type,created_at')
    ...
    safeRows(
      supabase
        .from('enrollments')
        .select('id,student_id,formation_id,status,enrolled_at')
  ...
  const activities = [
    ...paymentsRes.data.map((row) => ({
      ...
      action: String(row.payment_status || 'pending'),
      resource_type: `billing_payment:${row.provider || 'provider'}`,
      resource_id: row.id,
      user_id: row.user_id,
      changes: { purchase_type: row.purchase_type || 'subscription' },
    })),
    ...
    ...enrollmentsRes.data.map((row) => ({
      ...
      changes: { formation_id: row.formation_id },
    })),
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const [usersCountRes, formationsCountRes, paymentsRes, webhooksRes, auditLogsRes] = await Promise.all([
        ...
        safeCount(
          supabase.from('formations').select('id', { count: 'exact', head: true }).eq('status', 'published')
        ),
        safeRows(
          supabase
            .from('billing_payments')
      ...
      const confirmedPayments = paymentsRes.data.filter((p) => String(p.payment_status) === 'confirmed');
      const revenueConfirmed = confirmedPayments.reduce((acc, p) => acc + Number(p.price_amount || 0), 0);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      if (type === 'modules') {
        ...
        if (formationsErr && !isMissingRelationError(formationsErr)) throw formationsErr;

        const rows = (formations || []).map((row) => {
          const meta = row.meta && typeof row.meta === 'object' ? row.meta : {};
          return {
            id: row.id,
            title: row.title || 'Sans titre',
            page_slug: `formation/${row.id}`,
  ...
  const deleteContent = async (id) => {
    try {
      if (type === 'modules') {
        ...
        const { error: delErr } = await supabase.from('formations').delete().eq('id', id);
        if (delErr) throw delErr;
      }
