...
class QueryBuilder<T = any> {
  ...
  private async execute(): Promise<SupabaseResult<T>> {
    ...
    switch (this.tableName) {
      ...
      case 'cimolace_clients': {
        ...
        return apiCall(cimolaceBackofficeApi.listClients());
      }
      case 'cimolace_sites':
        return apiCall(cimolaceBackofficeApi.listSites());
  ...
  private async executeRealSupabase(): Promise<SupabaseResult<T>> {
    ...
    // These are typically: profiles, tenant_configs, and other utility tables
    let query: any;
    ...
    if (this.insertData) {
      ...
      query = realSupabase.from(this.tableName).select(this.selectColumns);
    }
