  ...
  const loadQueue = useCallback(async () => {
    ...
    try {
      const [billingRes, pendingEnrollmentsRes] = await Promise.all([
        ...
        supabaseClient
          .from('enrollments')
          .select('id,status,enrolled_at,student_id,formations(title)')
      ...
      const actorIds = [...new Set([...billingRows.map((r) => r.user_id), ...pendingRows.map((r) => r.student_id)].filter(Boolean))];
      let profileMap = {};
      ...
      const queueItems = [
        ...pendingRows.map((row) => ({
          ...
          targetId: row.id,
          type: 'inscription',
          title: row.formations?.title || 'Formation',
          subject: profileMap[row.student_id] || 'Étudiant',
          status: row.status || 'pending',
          date: row.enrolled_at || new Date().toISOString(),
          actionLabel: 'Traiter',
        })),
