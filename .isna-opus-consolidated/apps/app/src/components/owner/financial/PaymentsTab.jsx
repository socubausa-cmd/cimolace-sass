...
const PaymentsTab = () => {
  ...
  const refresh = useCallback(async () => {
    ...
    const { data: rows, error } = await supabase
      ...
      .select('id,user_id,payment_status,payment_method,payment_type,price_amount,price_currency,provider,order_id,provider_payment_id,created_at,paid_at')
    ...
    const mapped = (rows || []).map((row) => {
      ...
      return {
        ...row,
        reference: row.order_id || row.provider_payment_id || row.id,
        studentName: p.name || p.email || row.user_id,
  ...
  const stats = useMemo(() => {
    ...
    const pending = payments.filter((p) =>
      ['pending', 'confirming'].includes(String(p.payment_status || '').toLowerCase())
    ).length;
    const confirmedCount = payments.filter((p) => String(p.payment_status || '').toLowerCase() === 'confirmed').length;
    const totalsByCurrency = payments
      .filter((p) => String(p.payment_status || '').toLowerCase() === 'confirmed')
      .reduce((acc, p) => {
        const c = String(p.price_currency || 'EUR').toUpperCase();
  ...
  const updatePaymentStatus = async (id, nextStatus) => {
    ...
    if (nextStatus === 'confirmed') payload.paid_at = new Date().toISOString();
    await supabase.from('billing_payments').update(payload).eq('id', id);
    await refresh();
  };
