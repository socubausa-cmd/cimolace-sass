...
const StudentsManagementPanel = () => {
  ...
  const load = useCallback(async () => {
    ...
    const [profilesRes, enrollmentsRes, attendanceRes, disciplineRes] = await Promise.all([
      ...
      supabase
        .from('enrollments')
        .select('student_id,service_type,status,formation_id,formations(title)')
    ...
    // Build enrollment map: student_id → best enrollment
    const enrollMap = {};
    (enrollmentsRes.data || []).forEach((e) => {
      if (!enrollMap[e.student_id]) enrollMap[e.student_id] = e;
    });
    ...
    const mapped = (profilesRes.data || []).map((p) => {
      ...
      return {
        ...
        phone:       p.phone || '',
        serviceType: enroll?.service_type || 'academique',
        formation:   enroll?.formations?.title || null,
