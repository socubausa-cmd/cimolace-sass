    ...
    const run = async () => {
      ...
      try {
        ...
        const { data: formationRow, error: formationError } = await withTimeout(
          supabase.from('formations').select('id, title, description, status, cycle, duration_weeks, price, image_url, meta, created_at').eq('id', effectiveFormationId).maybeSingle(),
          10000,
          'Timeout formation'
        );
