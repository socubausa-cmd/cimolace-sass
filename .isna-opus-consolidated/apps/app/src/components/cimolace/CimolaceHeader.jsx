...
const navLinks = [
  ...
  { label: 'Documentation', href: '/cimolace/resources/documentation', isRoute: true },
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${bgClass}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="flex items-center justify-between h-20">
          {/* Logo */}
          <Link to="/cimolace" className="flex items-center gap-3">
            ...
            {navLinks.map((link) =>
              link.isRoute ? (
                ...
                <a
                  key={link.label}
                  href={link.href}
                  className={`px-3 py-2 text-sm font-medium ${mutedColor} ${hoverColor} hover:bg-black/5 rounded-lg transition-all duration-200`}
                >
                  {link.label}
                </a>
              )
