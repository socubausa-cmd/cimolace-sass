...
> 🚨 **DÉPLOIEMENT FRONT — RÈGLE ABSOLUE (tous agents) : `bash deploy-liri.sh` UNIQUEMENT, JAMAIS `vercel --prod` à la main.** Un déploiement manuel d'un HEAD committé peut rendre la prod **BLANCHE sur toutes les routes** (crash `undefined.default` — mélange import statique+dynamique, ex. `main.tsx → @/App`). `deploy-liri.sh` fait un **smoke test runtime + rollback auto** avant de basculer `origin/main` ; une **surveillance** (LaunchAgent) rollback aussi automatiquement si la prod casse. Détails → §« DÉPLOIEMENT PROD » règle 6 + mémoire `vite-prod-crash-build-only`.
>
...
>
> 🧭 **REPRISE CLAUDE — Liri Portail / Prorascience** : lire [docs/LIRI_PORTAIL_CLAUDE_HANDOFF_2026-07-24.md](docs/LIRI_PORTAIL_CLAUDE_HANDOFF_2026-07-24.md) avant toute suite sur tunnel de vente, Mes cours, player post-production, forum, messagerie ou live immersif. Ne pas revenir à l'ancien design : l'interface officielle est Liri Portail `/liri`.
