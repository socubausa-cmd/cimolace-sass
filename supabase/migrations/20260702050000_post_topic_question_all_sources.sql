-- Étend post_topic_question à TOUTES les sources vidéo du picker forum :
-- live (existant), cours numérique/courses (existant), + precepteur (course_render_jobs),
-- masterclass (masterclass_lessons), cursus (formation_day_contents → jusqu'à courses).
-- Additif : seul le bloc de résolution du tenant change ; le reste (garde membre,
-- get-or-create du Sujet, participant, clip, message) est identique.

-- Le CHECK context_type n'autorisait que video|live|class → on ajoute les nouvelles
-- sources vidéo (course = cours numérique, cursus, precepteur, masterclass).
ALTER TABLE public.conversations DROP CONSTRAINT IF EXISTS conversations_context_type_chk;
ALTER TABLE public.conversations ADD CONSTRAINT conversations_context_type_chk
  CHECK (context_type IS NULL OR context_type = ANY (ARRAY['video','live','class','course','cursus','precepteur','masterclass']));

CREATE OR REPLACE FUNCTION public.post_topic_question(
  p_context_type text,
  p_context_id uuid,
  p_question text,
  p_clip_start numeric DEFAULT NULL::numeric,
  p_clip_end numeric DEFAULT NULL::numeric,
...
BEGIN
  ...
  -- Résolution du tenant selon la SOURCE vidéo (chaque source expose un tenant).
  IF p_context_type = 'live' THEN
    SELECT tenant_id INTO v_tenant FROM public.live_sessions WHERE id = p_context_id;
  ELSIF p_context_type = 'precepteur' THEN
    SELECT tenant_id INTO v_tenant FROM public.course_render_jobs WHERE id = p_context_id;
  ELSIF p_context_type = 'masterclass' THEN
    SELECT tenant_id INTO v_tenant FROM public.masterclass_lessons WHERE id = p_context_id;
  ELSIF p_context_type = 'cursus' THEN
    -- Cursus école : formation_day_contents → formation_days → formation_weeks
    -- → modules → courses (modules.formation_id) → tenant.
    SELECT c.tenant_id INTO v_tenant
    FROM public.formation_day_contents fdc
    JOIN public.formation_days fd ON fd.id = fdc.day_id
    ...
    WHERE fdc.id = p_context_id;
  ELSE
    -- 'course' / 'video' : le contexte est un cours numérique.
