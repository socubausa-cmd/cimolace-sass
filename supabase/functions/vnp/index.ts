...
async function callChat(message: string, platformName: string, graph: string, history: Array<{ role: string; content: string }>) {
  ...
  const system =
    ...
    `MISE EN SCÈNE — LE RENDU PAR DÉFAUT : dès que ta réponse EXPLIQUE, COMPARE, LISTE, STRUCTURE ou RACONTE (donc presque toujours), tu COMPOSES une scène designée qui illustre la réponse — ne te contente PAS d'un nodeId + texte plat. Le texte seul (scene=null) est réservé aux réponses ultra-courtes : un fait isolé, une confirmation, un refus hors-sujet. ` +
    ...
    `QUAND : « X vs Y / en quoi diffère… / comparez » → split ; « vos piliers / garanties / valeurs / pour qui » → aside ; « comment ça marche / par où commencer / que vais-je apprendre » → tutorial ou timeline ; « votre histoire / fondateur » → reader.\n` +
    `EXEMPLE (forme, à REMPLIR avec le graphe) : Q « en quoi diffère-t-on d'une école classique ? » → scene={"type":"split","headline":"…","left":{"title":"École classique","subtitle":"…","points":["…","…"]},"right":{"title":"${platformName}","subtitle":"…","points":["…","…"]},"tone":{"left":"terra","right":"gold"}}.\n` +
    `NE COMPOSE JAMAIS les prix, chiffres ou comparatifs toi-même : pour les FORFAITS mets nodeId="produits", pour les CHIFFRES nodeId="realisations", pour COMPARER les cycles nodeId="solutions" — le site les rend depuis la donnée VÉRIFIÉE (ne fabrique pas de scène cards/stats/comparateur). Pour les QUESTIONS FRÉQUENTES / la FAQ mets nodeId="faq" et scene=null (le site rend un accordéon depuis les vraies Q&R). Si aucune scène fiable n'est possible, scene=null (le texte suffit).\n\n` +
